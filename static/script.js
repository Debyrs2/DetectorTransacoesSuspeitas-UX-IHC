...
$('btnFeedback')?.addEventListener('click', () => {
    ...
    const conteudoFeedback = `
        ...
        <textarea id="feedbackText" placeholder="${dicionarioAtual.feedbackPlaceholder || 'Conte-nos o que achou da plataforma (opcional)'}" style="box-sizing: border-box; width: 100%; height: 90px; padding: 10px; border-radius: 8px; border: 1px solid var(--line); background: var(--bg-0); color: var(--text); resize: none; margin-bottom: 10px;"></textarea>
    `;
    ...
    abrirModalGenerico(dicionarioAtual.feedbackTitle || "Avalie o DataGuard", conteudoFeedback, false, "", "brand", async () => {
        ...
        if (rating === 0) {
            ...
            return false; // Retornar false avisa o modal genérico para NÃO fechar a tela
        }
        ...
        try {
            ...
            return false; // Se a internet cair, não fecha o modal para ele tentar de novo
        }
    });

    $('btnGenConfirm').textContent = dicionarioAtual.btnSubmitFeedback || "Enviar Avaliação";

    // Altera o texto de Cancelar especificamente para esta tela
    $('btnGenCancel').textContent = "Responder mais tarde";
